...
def main():
	...
	while sliding_window_start + samples_per_window < audio_length:
		...
		segment_offscreen = audio_offscreen[sliding_window_start:sliding_window_end]	
		...
		audio_segment = (segment1_audio + segment2_audio + segment_offscreen * opt.noise_weight) / 3
	...
	librosa.output.write_wav(os.path.join(output_dir, 'audio_offscreen.wav'), audio_offscreen, opt.audio_sampling_rate)
