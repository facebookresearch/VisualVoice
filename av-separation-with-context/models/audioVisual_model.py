...
class AudioVisualModel(torch.nn.Module):
    ...
    def forward(self, input):
        ...
        if self.opt.visual_feature_type == 'both':
            visual_feature_1 = torch.cat((identity_feature_A, lipreading_feature_A1, identity_feature_B, lipreading_feature_B), dim=1)
            visual_feature_2 = torch.cat((identity_feature_A, lipreading_feature_A2, identity_feature_B, lipreading_feature_B), dim=1)
        elif self.opt.visual_feature_type == 'lipmotion':
            visual_feature_1 = torch.cat((lipreading_feature_A1, lipreading_feature_B), dim=1)
            visual_feature_2 = torch.cat((lipreading_feature_A2, lipreading_feature_B), dim=1)
        elif self.opt.visual_feature_type == 'identity':
            visual_feature_1 = torch.cat((identity_feature_A, identity_feature_B), dim=1)
            visual_feature_2 = torch.cat((identity_feature_A, identity_feature_B), dim=1)
        ...
        mask_prediction_1 = scalar * self.net_unet(audio_mix_spec1, visual_feature_1, activation)
        mask_prediction_2 = scalar * self.net_unet(audio_mix_spec2, visual_feature_2, activation)
        mask_prediction_A1 = mask_prediction_1[:,0:2,:,:]
        mask_prediction_A2 = mask_prediction_2[:,0:2,:,:]
        mask_prediction_B1 = mask_prediction_1[:,2:4,:,:]
        mask_prediction_B2 = mask_prediction_2[:,2:4,:,:]
